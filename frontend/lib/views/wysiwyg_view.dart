...
class _WysiwygViewState extends ConsumerState<WysiwygView> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          _buildTopBar(themeConfigAsync.value, ref.read(activeThemeConfigProvider.notifier), theme, isDark),
  ...
  Widget _buildTopBar(ThemeConfigModel? config, ActiveThemeConfigNotifier notifier, ThemeData theme, bool isDark) {
    return ClipRect(
            ...
            children: [
                  ...
                  Row(
                    children: [
                      ...
                      Container(
                        ...
                        child: const Text('DRAFT', style: TextStyle(color: AppTheme.brandEmerald500, fontSize: 9, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                  const Text(
              ...
              HoverScale(
                child: ElevatedButton.icon(
                  ...
                  label: const Text('Publish Changes', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
  ...
  Widget _buildSidebar(AsyncValue<dynamic> configAsync, ThemeData theme, bool isDark) {
    return Container(
      width: 340,
      decoration: BoxDecoration(
        ...
        data: (config) => ListView(
          ...
          children: [
            _buildSidebarSection(
              ...
              isDark: isDark,
              children: [
                _colorPickerTile('Primary Theme Color', 'primary', config?.draftTokens['primary'], theme),
                const SizedBox(height: 8),
                _colorPickerTile('Secondary Tone Color', 'secondary', config?.draftTokens['secondary'], theme),
                const SizedBox(height: 8),
                _colorPickerTile('Canvas Background', 'background', config?.draftTokens['background'], theme),
              ],
            ),
            ...
            _buildSidebarSection(
              ...
              isDark: isDark,
              children: [
                _buildSlotTextEditor('Hero Title', 'hero_heading', config?.draftSlots['hero_heading'], theme),
                const SizedBox(height: 16),
                _buildSlotTextEditor('Hero Subtitle', 'hero_subheading', config?.draftSlots['hero_subheading'], theme),
                const SizedBox(height: 16),
                _buildSlotDropdown('Hero Alignment', 'hero', config?.draftSlots['hero'], ['full-width', 'centered'], theme),
              ],
            ),
          ],
        ),
  ...
  Widget _colorPickerTile(String label, String key, String? hex, ThemeData theme) {
    ...
    return ListTile(
      ...
      subtitle: Text(hex ?? '#000000', style: TextStyle(color: theme.hintColor, fontSize: 11)),
      trailing: Container(
        width: 32,
        height: 32,
        decoration: BoxDecoration(
  ...
  Widget _buildSlotTextEditor(String label, String key, String? value, ThemeData theme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: AppTheme.brandEmerald500)),
          ),
  ...
  Widget _buildSlotDropdown(String label, String key, String? value, List<String> options, ThemeData theme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: TextStyle(fontSize: 12, color: theme.hintColor, fontWeight: FontWeight.w600)),
  ...
  Widget _buildPreviewCanvas(AsyncValue<dynamic> configAsync, ThemeData theme, bool isDark) {
    return Container(
  ...
  Widget _buildBrowserWrapper({required Widget child, required bool isDark, required ThemeData theme}) {
    return Container(
      decoration: BoxDecoration(
        color: isDark ? const Color(0xFF0F172A) : Colors.white,
        borderRadius: BorderRadius.circular(16),
  ...
  Widget _buildMobilePhoneWrapper({required Widget child, required bool isDark, required ThemeData theme}) {
    return Container(
      width: 380,
      height: 720,
      decoration: BoxDecoration(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Theme published successfully!'), backgroundColor: AppTheme.brandEmerald600),
        );
      }
    } catch (e) {
      if (mounted) {
