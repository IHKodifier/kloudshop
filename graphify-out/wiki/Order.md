...
> God node · 49 connections · [E:\Non_Office\Dev_Space\vibe_skool\kloudShop\backend\modules\orders\models.py](file:///E:/Non_Office/Dev_Space/vibe_skool/kloudShop/backend/modules/orders/models.py#L9)
...
sequenceDiagram
    ...
    participant P197 as Test end-to-end checkout: payment-intent -> confirm -> inventory decrement.
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
...
- [[MockStripe]] `INFERRED`
- [[Generate Facebook OAuth authorization URL.]] `INFERRED`
- [[Invite a new staff member.]] `INFERRED`
- [[List pending invitations for the current tenant.]] `INFERRED`
- [[Cancel a pending invitation.]] `INFERRED`
- [[Update a staff member's roles and sync with Firebase claims.]] `INFERRED`
- [[Revoke a staff member's access to the current tenant.]] `INFERRED`
- [[Transfer store ownership to another staff member.]] `INFERRED`
- [[Register a B2B buyer using an invitation token.]] `INFERRED`
- [[Register a DTC consumer (post-checkout).]] `INFERRED`
- [[Checks the active cool-off and blocked status of a Gmail account.]] `INFERRED`
- [[Logs a failed sign-in attempt and initiates a 180s cool-off or block.]] `INFERRED`
- [[Sends a short-lived (180s TTL) unblock token email to the user (max 3/24h).]] `INFERRED`
- [[Verifies the unblock token and unlocks the account (does not require login).]] `INFERRED`
- [[Allows an administrator or store owner to unblock a locked staff account.]] `INFERRED`
- [[Step 1: Calculate total and create Stripe PaymentIntent.     Public storefront]] `INFERRED`
