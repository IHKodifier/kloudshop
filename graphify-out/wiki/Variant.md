...
> God node · 104 connections · [E:\Non_Office\Dev_Space\vibe_skool\kloudShop\backend\modules\catalog\models.py](file:///E:/Non_Office/Dev_Space/vibe_skool/kloudShop/backend/modules/catalog/models.py#L84)
...
sequenceDiagram
    ...
    participant P205 as _process_import_job()
    participant P206 as seed_demo_data()
    participant P207 as test_get_b2b_catalog_overrides()
    participant P208 as test_pos_sale_success()
    participant P209 as ChannelAdapter
    participant P210 as TikTokAdapter
    participant P211 as InstagramAdapter
    participant P212 as FacebookAdapter
    participant P213 as test_analytics_needs_attention()
    participant P214 as test_pos_sale_insufficient_stock()
    participant P215 as ChannelSyncService
    participant P216 as update_product()
    participant P217 as test_place_b2b_order_threshold()
    participant P218 as Test end-to-end checkout: payment-intent -> confirm -> inventory decrement.
    participant P219 as create_product()
    participant P220 as test_shipping_calculation_blending_success()
    participant P221 as test_order_checkout_flow()
    participant P222 as test_tax_calculation_manual_fallback()
    participant P223 as background_import_task()
    participant P224 as test_tiktok_sync()
    participant P225 as test_instagram_sync()
    participant P226 as test_facebook_sync()
    participant P227 as test_purchase_order_lifecycle()
    participant P228 as test_stock_transfer()
    participant P229 as test_insufficient_stock()
    participant P230 as test_jsonld_structured_data()
    participant P231 as test_sitemap_xml()
    participant P232 as test_tax_calculation_stripe_automated()
    participant P233 as CSV-B-001 & CSV-B-002: Verify template returns valid headers.
    participant P234 as CSV-B-003: Check SKU existence check works.
    participant P235 as CSV-B-004 & CSV-B-010: Successful import of clean CSV.
    participant P236 as CSV-B-005: Upload with conflict_strategy=skip skips duplicate SKUs.
    participant P237 as CSV-B-006: Upload with conflict_strategy=overwrite updates existing variant.
    participant P238 as CSV-B-007: Upload with conflict_strategy=custom_sku renames duplicate SKU.
    participant P239 as CSV-B-008, CSV-B-009, CSV-B-011, CSV-B-013: Verify error reporting and validatio
    participant P240 as CSV-B-012: Rows with no stock are logged to no_stock_log.
    participant P241 as CSV-B-014: Parse arbitrary option columns (e.g. Option7 Name/Value).
    participant P242 as CSV-B-015: History endpoint returns jobs for tenant sorted by date desc.
    participant P243 as CSV-B-016: Parse XLSX file bytes successfully.
    participant P244 as test_sku_exists_endpoint()
    participant P245 as test_import_conflict_skip()
    participant P246 as test_import_conflict_overwrite()
    participant P247 as test_import_conflict_custom_sku()
    participant P248 as Generates a live Google Shopping XML feed for the tenant.
    participant P249 as test_google_shopping_feed()
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
...
- [[test_shipping_calculation_blending_success()]] `INFERRED`
- [[test_order_checkout_flow()]] `INFERRED`
- [[test_tax_calculation_manual_fallback()]] `INFERRED`
