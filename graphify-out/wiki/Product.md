...
> God node · 73 connections · [E:\Non_Office\Dev_Space\vibe_skool\kloudShop\backend\modules\catalog\models.py](file:///E:/Non_Office/Dev_Space/vibe_skool/kloudShop/backend/modules/catalog/models.py#L10)
...
sequenceDiagram
    ...
    participant P197 as ChannelAdapter
    participant P198 as TikTokAdapter
    participant P199 as InstagramAdapter
    participant P200 as FacebookAdapter
    participant P201 as ChannelSyncService
    participant P202 as Test end-to-end checkout: payment-intent -> confirm -> inventory decrement.
    participant P203 as Generates a dynamic XML sitemap for the storefront.
    participant P204 as CSV-B-001 & CSV-B-002: Verify template returns valid headers.
    participant P205 as CSV-B-003: Check SKU existence check works.
    participant P206 as CSV-B-004 & CSV-B-010: Successful import of clean CSV.
    participant P207 as CSV-B-005: Upload with conflict_strategy=skip skips duplicate SKUs.
    participant P208 as CSV-B-006: Upload with conflict_strategy=overwrite updates existing variant.
    participant P209 as CSV-B-007: Upload with conflict_strategy=custom_sku renames duplicate SKU.
    participant P210 as CSV-B-008, CSV-B-009, CSV-B-011, CSV-B-013: Verify error reporting and validatio
    participant P211 as CSV-B-012: Rows with no stock are logged to no_stock_log.
    participant P212 as CSV-B-014: Parse arbitrary option columns (e.g. Option7 Name/Value).
    participant P213 as CSV-B-015: History endpoint returns jobs for tenant sorted by date desc.
    participant P214 as CSV-B-016: Parse XLSX file bytes successfully.
    participant P215 as Generates a live Google Shopping XML feed for the tenant.
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P2: uses
    P2-->>- P1: return
